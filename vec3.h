...
// point3 is just an alias for vec3, useful for geometric clarity in the code
using point3 = vec3;
...
inline vec3 random_on_hemisphere(const vec3& normal) {
   ...
   return (dot(on_unit_sphere, normal) > 0.0) ? on_unit_sphere
                                              : -on_unit_sphere;
}
...
inline vec3 refract(const vec3& uv, const vec3& n, double etai_over_etat) {
   // uv and n are assumed to be unit vectors
